...
- Use the full scoring range from 1 to 5.
- Avoid assigning the same score across all reports unless clearly justified.
- Average-quality reports should typically receive a score of 3.
...
The report summarizes company performance but provides vague statements such as "growth is strong" without evidence. Risks are barely discussed, and the recommendation is generic.

Output:
{{
  "accuracy": {{"score": 3, "reason": "mostly correct but lacks supporting evidence"}},
  "depth": {{"score": 2, "reason": "mostly descriptive with weak reasoning"}},
  "risk": {{"score": 2, "reason": "only limited risks mentioned"}},
  "coherence": {{"score": 2, "reason": "recommendation is generic and weakly justified"}}
}}

Example 2 (Average Report):
The report includes correct financial information and some reasoning but lacks deeper insight. Risks are mentioned but not fully explored. Recommendation is reasonable.

Output:
{{
  "accuracy": {{"score": 4, "reason": "accurate with minor issues"}},
  "depth": {{"score": 3, "reason": "moderate reasoning but not very deep"}},
  "risk": {{"score": 3, "reason": "some key risks covered but incomplete"}},
  "coherence": {{"score": 3, "reason": "reasonable but not strongly justified"}}
}}

Example 3 (Strong Report):
The report provides detailed financial reasoning, clear supporting evidence, and well-explained risks across multiple categories. The recommendation is strongly justified.

Output:
{{
  "accuracy": {{"score": 5, "reason": "highly accurate and well-supported"}},
  "depth": {{"score": 5, "reason": "strong and insightful reasoning"}},
  "risk": {{"score": 5, "reason": "comprehensive and balanced risk coverage"}},
  "coherence": {{"score": 5, "reason": "clear and well-justified recommendation"}}
}}
...
# COHEN'S KAPPA
# =========================================
def bin_score(x): # for binned kapa (more flexible instead of must match exact rating score)
    if x <= 2:
        return "low"
    elif x <= 4:
        return "medium"
...
def compute_kappa(llm_df: pd.DataFrame, human_df: pd.DataFrame) -> str:
    ...
    for d in DIMENSIONS:
        ...
        # Binned kappa
        merged[f"{d}_human_bin"] = merged[f"{d}_human"].apply(bin_score)
        merged[f"{d}_llm_bin"] = merged[f"{d}_llm"].apply(bin_score)

        kappa_binned = cohen_kappa_score(
            merged[f"{d}_human_bin"],
            merged[f"{d}_llm_bin"]
        )

        lines.append(f"- {d} (raw): {kappa:.3f}")
        lines.append(f"- {d} (binned): {kappa_binned:.3f}")
